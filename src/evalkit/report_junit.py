...
def _add_case(parent: ET.Element, suite_name: str, case: CaseResult) -> None:
    ...
    if case.status == "fail":
        messages = [f.message for f in case.failures]
        ...
        if case.response_excerpt:
            ...
            body.append(case.response_excerpt[:RESPONSE_EXCERPT_CHARS])
        failure.text = "\n".join(body)
    elif case.status == "error":
        error = ET.SubElement(testcase, "error", {"message": case.error or "error"})
        error.text = case.error or ""
